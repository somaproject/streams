...
WaveVisTrigger::WaveVisTrigger(std::vector<GLWavePoint_t> * pSamples) :
  pSamples_(pSamples), 
  triggerLevel_(0.8), 
...
bool WaveVisTrigger::triggerFunc(GLWavePoint_t p1,  GLWavePoint_t p2)
{
  // p1 comes before p2:
...
void WaveVisTrigger::enableTrigger(bool state)
{
...
void WaveVisTrigger::setTriggerValue(float val)
{
  triggerLevel_ = val;
}

QueueView<float> WaveVisTrigger::getTriggerQueueView()
{
  return QueueView<float>(triggers_); 
}

updateTriggersSignal_t & WaveVisTrigger::updateTriggersSignal()
{
