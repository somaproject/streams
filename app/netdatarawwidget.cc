...
NetDataRawWidget::NetDataRawWidget(NetDataRaw * nw, pSomaConfig_t sc) :
  ElementStatus(sc), 
  netDataWave_(nw)
{
  add(vboxMain_); 
...
//   labelHPF_.set_label("HP Filter"); 
  ...
  netDataWave_->src.signal().connect(sigc::mem_fun(*this, 
						  &NetDataRawWidget::sourceCallback)); 
  gainCallback(0); 
  netDataWave_->gain.signal().connect(sigc::mem_fun(*this, 
						    &NetDataRawWidget::gainCallback)); 
    
  hpfCallback(false); 
  netDataWave_->hpfen.signal().connect(sigc::mem_fun(*this, 
						     &NetDataRawWidget::hpfCallback)); 
  
}


NetDataRawWidget::~NetDataRawWidget()
{
  
  
}

void NetDataRawWidget::sourceCallback(int src)
{
...
void NetDataRawWidget::gainCallback(int gain)
{
...
void NetDataRawWidget::hpfCallback(bool hpf)
{
